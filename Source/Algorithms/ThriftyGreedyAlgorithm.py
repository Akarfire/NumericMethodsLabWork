...
class ThriftyGreedyAlgorithm(Algorithm):
    ...
    def run(input_data: Data) -> None:
        ...
        stages = input_data.n
        thrifty_stages = input_data.thrifty_greedy_stage - 1
        ...
        input_data.statistics.sugarity_data_per_algorithm[AlgorithmNames.THRIFTY_GREEDY] = sugarity_per_stage
...
def test():
    matrix = [[1, 1, 1456456, 100, 1],
              ...
              [4, 4, 4,       4,   4],
              [5, 5, 5234234, 5,   5]]
    data = Data(n=5, thrifty_greedy_stage=3, matrix=matrix)
    ThriftyGreedyAlgorithm.run(data)

    print(data.statistics.sugarity_data_per_algorithm)
    prev = 0
    print("Choosing order: ",end="")
    for i in data.statistics.sugarity_data_per_algorithm[AlgorithmNames.THRIFTY_GREEDY]:
        print(i - prev, end=" ")
        prev = i
    print()
    print(data.matrix)
